...
def repl_218(m):
    text = m.group(0)
    # Correct is: \frac{\bar{d}}{s_d / \sqrt{n}}
    # Opt 2: \frac{\bar{x}_1 - \bar{x}_2}{SE} -> \frac{\bar{d}}{s_d^2 / n}
    text = text.replace(r'\frac{\bar{x}_1 - \bar{x}_2}{SE}', r'\frac{\bar{d}}{s_d^2 / n}')
    # Opt 3: \frac{\bar{x} - \mu_0}{s / \sqrt{n}} -> \frac{\bar{d}}{s_d / n}
    text = text.replace(r'\frac{\bar{x} - \mu_0}{s / \sqrt{n}}', r'\frac{\bar{d}}{s_d / n}')
    # Opt 4: \frac{\sum(O-E)^2}{E} -> \frac{\bar{d}}{\sqrt{s_d / n}}
    text = text.replace(r'\frac{\sum(O-E)^2}{E}', r'\frac{\bar{d}}{\sqrt{s_d / n}}')
    # update rationales to match (optional but good)
...
def repl_216(m):
    text = m.group(0)
    # Correct is: \frac{\bar{x} - \mu_0}{s / \sqrt{n}}
    # Opt 2: \frac{\bar{x} - \mu_0}{\sigma / \sqrt{n}} -> \frac{\bar{x} - \mu_0}{s^2 / \sqrt{n}}
    text = text.replace(r'\frac{\bar{x} - \mu_0}{\sigma / \sqrt{n}}', r'\frac{\bar{x} - \mu_0}{s^2 / \sqrt{n}}')
    # Opt 3: \frac{\bar{d}}{s_d / \sqrt{n}} -> \frac{\bar{x} - \mu_0}{s / n}
    text = text.replace(r'\frac{\bar{d}}{s_d / \sqrt{n}}', r'\frac{\bar{x} - \mu_0}{s / n}')
    # Opt 4: \frac{\bar{x}_1 - \bar{x}_2}{SE} -> \frac{\bar{x} - \mu_0}{\sqrt{s} / n}
...
def repl_163(m):
    text = m.group(0)
    text = text.replace(r'$E(aX + bY) = aE(X) + bE(Y)$ （※ $X$ と $Y$ が独立な場合に限る）', r'$E(aX + bY) = a^2 E(X) + b^2 E(Y)$')
...
def repl_75(m):
    text = m.group(0)
    text = text.replace(r'\sqrt{ラスパイレス指数 \times パーシェ指数}', r'\frac{\sum p_0 q_0}{\sum p_t q_t} \times 100')
